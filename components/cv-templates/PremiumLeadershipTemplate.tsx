...
export default function PremiumLeadershipTemplate({ data }: { data: CvRenderData }) {
  ...
  return (
    <div style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: 12, width: "100%", color: "#1a1a1a", background: "#fff", height: "100%", display: "flex", flexDirection: "column" }}>
      {/* Header premium sombre */}
      <div style={{ background: ac, color: "#fff", padding: "40px 40px 30px", display: "flex", alignItems: "center", gap: 24 }}>
        {photo && i.photoUrl && (
          <img src={i.photoUrl} alt="" style={{ width: 100, height: 125, borderRadius: "8px", objectFit: "cover", border: "3px solid rgba(255,255,255,0.3)", flexShrink: 0 }} />
        )}
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, margin: 0, letterSpacing: 1 }}>{i.fullName || ""}</h1>
          <p style={{ fontSize: 15, opacity: 0.85, margin: "6px 0 4px", fontWeight: 600 }}>{i.title || ""}</p>
          {adapted && targetLabel && (
            <p style={{ fontSize: 10, opacity: 0.65, margin: "0 0 8px", fontStyle: "italic" }}>Candidature : {targetLabel}</p>
          )}
      ...
      <div style={{ padding: "36px 40px", display: "flex", gap: 30, flex: 1 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          {data.experiences.length > 0 && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
              <h2 style={{ fontSize: 13, fontWeight: 700, color: ac, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 }}>Expériences professionnelles</h2>
              <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                {data.experiences.map((exp, j) => (
                  <div key={j} style={{ position: "relative", paddingLeft: 20 }}>
                    ...
                    {(exp.bullets || exp.achievements)?.slice(0, 4).map((b, k) => <p key={k} style={{ fontSize: 11, color: "#555", margin: "2px 0 2px 8px" }}>• {b}</p>)}
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ width: 220, flexShrink: 0 }}>
          ...
          {data.achievements.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              ...
              {data.achievements.map((a, j) => (
                <div key={j} style={{ background: accentBg(data, 0.06), padding: "10px 12px", borderRadius: 6, marginBottom: 6, borderLeft: `3px solid ${ac}` }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {data.skills.map((s, j) => {
                  ...
                  return (
                    <span key={j} style={{ fontSize: 11, padding: "4px 8px", background: isMatch ? accentBg(data, 0.15) : accentBg(data, 0.06), borderRadius: 3, fontWeight: isMatch ? 600 : 400, borderLeft: isMatch ? `2px solid ${ac}` : "none" }}>{s}</span>
                  );
                })}
              </div>
            </div>
          ...
          {data.education.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              ...
              {data.education.map((e, j) => (
                <p key={j} style={{ fontSize: 11, margin: "4px 0", lineHeight: 1.4 }}><strong>{e.degree}</strong><br />{e.school}{e.year ? ` (${e.year})` : ""}</p>
              ))}
            </div>
          )}
          ...
          {data.languages.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              ...
              {data.languages.map((l, j) => (
                <p key={j} style={{ fontSize: 11, margin: "3px 0" }}>{l.name}{l.level ? ` — ${l.level}` : ""}</p>
              ))}
            </div>
          )}
          ...
          {data.certifications.length > 0 && (
            <div>
              ...
              {data.certifications.map((c, j) => <p key={j} style={{ fontSize: 11, margin: "2px 0" }}>• {c}</p>)}
            </div>
          )}
        </div>
      </div>
